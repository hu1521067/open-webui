from tc_webui.retrieval.vector.utils import stringify_metadata
from tc_webui.retrieval.vector.main import (
    VectorDBBase,
    VectorItem,
    GetResult,
    SearchResult,
)
from tc_webui.config import S3_VECTOR_BUCKET_NAME, S3_VECTOR_REGION
...
class S3VectorClient(VectorDBBase):
    """
    AWS S3 Vector integration for TC WebUI Knowledge.
    """
    ...
    def _filter_metadata(
        ...
        # Keep only the first 10 keys, prioritizing important ones based on actual TC WebUI metadata
        important_keys = [
    ...
    def get(self, collection_name: str) -> Optional[GetResult]:
        ...
        try:
            ...
            # The TC WebUI GetResult expects lists of lists, so we wrap each list
            if all_ids:
                return GetResult(
