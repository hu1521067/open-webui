...
from tc_webui.config import CACHE_DIR
from tc_webui.constants import ERROR_MESSAGES
from tc_webui.env import ENABLE_FORWARD_USER_INFO_HEADERS, SRC_LOG_LEVELS
from tc_webui.routers.files import upload_file_handler
from tc_webui.utils.auth import get_admin_user, get_verified_user
from tc_webui.utils.images.comfyui import (
    ...
    try:
        if request.app.state.config.IMAGE_GENERATION_ENGINE == "openai":
            ...
            if ENABLE_FORWARD_USER_INFO_HEADERS:
                headers["X-TCWebUI-User-Name"] = quote(user.name, safe=" ")
                headers["X-TCWebUI-User-Id"] = user.id
                headers["X-TCWebUI-User-Email"] = user.email
                headers["X-TCWebUI-User-Role"] = user.role
