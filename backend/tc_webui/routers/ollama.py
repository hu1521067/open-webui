...
from tc_webui.utils.payload import (
        ...
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(
                ...
                headers={
                    ...
                    **(
                        {
                            "X-TCWebUI-User-Name": quote(user.name, safe=" "),
                            "X-TCWebUI-User-Id": user.id,
                            "X-TCWebUI-User-Email": user.email,
                            "X-TCWebUI-User-Role": user.role,
                        }
...
async def send_post_request(
    ...
    try:
                ...
                **(
                    {
                        ...
                        "X-TCWebUI-User-Role": user.role,
                        **(
                            {"X-TCWebUI-Chat-Id": metadata.get("chat_id")}
        ...
        if r.ok is False:
            try:
                ...
                raise HTTPException(
                    ...
                    detail=f"TC WebUI: Server Connection Error",
                )
        ...
        raise HTTPException(
            ...
            detail=detail if e else "TC WebUI: Server Connection Error",
        )
    finally:
        if not stream:
            await cleanup_response(r, session)
...
async def verify_connection(
    ...
    async with aiohttp.ClientSession(
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail="TC WebUI: Server Connection Error"
            )
        except Exception as e:
    ...
    if url_idx is None:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=detail if detail else "TC WebUI: Server Connection Error",
            )
...
async def get_openai_models(
    ...
    if url_idx is None:
        ...
        try:
            ...
            log.exception(e)
            error_detail = "TC WebUI: Server Connection Error"
            if r is not None:
                try:
                    res = r.json()
                    if "error" in res:
