...
# TC_WEBUI_DIR should be the directory where env.py resides (tc_webui/)
TC_WEBUI_DIR = ENV_FILE_PATH.parent

# BACKEND_DIR is the parent of TC_WEBUI_DIR (backend/)
BACKEND_DIR = TC_WEBUI_DIR.parent

# BASE_DIR is the parent of BACKEND_DIR (tc-webui-dev/)
...
WEBUI_NAME = os.environ.get("WEBUI_NAME", "知识问答")

WEBUI_FAVICON_URL = "https://tcwebui.com/favicon.png"
...
if FROM_INIT_PY:
    PACKAGE_DATA = {"version": importlib.metadata.version("tc-webui")}
else:
    try:
...
#
# except Exception:
#     changelog_content = (pkgutil.get_data("tc_webui", "CHANGELOG.md") or b"").decode()
#
...
# Fallback CHANGELOG definition for Docker deployment
CHANGELOG = {}
...
if FROM_INIT_PY:
    NEW_DATA_DIR = Path(os.getenv("DATA_DIR", TC_WEBUI_DIR / "data")).resolve()
    ...
    if DATA_DIR.exists() and DATA_DIR != NEW_DATA_DIR:
        ...
        shutil.make_archive(DATA_DIR.parent / "tc_webui_data", "zip", DATA_DIR)

        # Remove the old data directory
        shutil.rmtree(DATA_DIR)

    DATA_DIR = Path(os.getenv("DATA_DIR", TC_WEBUI_DIR / "data"))

STATIC_DIR = Path(os.getenv("STATIC_DIR", TC_WEBUI_DIR / "static"))

FONTS_DIR = Path(os.getenv("FONTS_DIR", TC_WEBUI_DIR / "static" / "fonts"))

FRONTEND_BUILD_DIR = Path(os.getenv("FRONTEND_BUILD_DIR", BASE_DIR / "build")).resolve()

if FROM_INIT_PY:
    FRONTEND_BUILD_DIR = Path(
        os.getenv("FRONTEND_BUILD_DIR", TC_WEBUI_DIR / "frontend")
    ).resolve()
...
REDIS_KEY_PREFIX = os.environ.get("REDIS_KEY_PREFIX", "tc-webui")
...
OTEL_SERVICE_NAME = os.environ.get("OTEL_SERVICE_NAME", "tc-webui")
